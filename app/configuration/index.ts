...
export const log = logger.child({ component: 'config' });
...
function getConfig(): IBaseConfig {
    ...
    if (process.env.WUD_LOG_LEVEL) {
        const level = process.env.WUD_LOG_LEVEL;
        if (logLevels.includes(level as never)) {
            log.setLogLevel(level as never);
        }
    }

    const previousConfig = cachedConfig;
    const config = processConfigFileOrDefault(previousConfig);
    ...
    Object.keys(wudEnvVars).forEach((wudEnvVar) => {
        ...
        if (configItem instanceof Map) {
            ...
            log.warn(
                `Invalid configuration for key: ${key}, expected an object but got ${typeof configItem}`,
            );
        }
    });
...
function processConfigFileOrDefault(previousConfig?: IBaseConfig): IBaseConfig {
    ...
    if (!fs.existsSync(configFilePath)) {
        if (configFilePath !== defaultConfigFile) {
            log.warn(
                `Config file ${configFilePath} does not exist, using only environment variables`,
            );
        } else {
            log.info(
                `Default config file ${configFilePath} does not exist, using only environment variables`,
            );
        }
        return config;
    }
    ...
    if (!configFileValue || configFileValue.length === 0) {
        ...
        if (previousConfig) {
            log.warn(
                `Config file ${configFilePath} is invalid, keeping previous configuration in memory; restarting the container will lose this fallback configuration`,
            );
    ...
    try {
        ...
        log.error(
            `Failed to load configuration file ${configFilePath}: ${errorMessage}`,
        );
        if (previousConfig) {
            log.warn(
                `Config file ${configFilePath} is invalid, keeping previous configuration in memory; restarting the container will lose this fallback configuration`,
    ...
    if (typeof configFileJson !== 'object' || configFileJson === null) {
        log.error(
            `Config file ${configFilePath} does not contain a valid JSON object`,
        );
        if (previousConfig) {
            log.warn(
                `Config file ${configFilePath} is invalid, keeping previous configuration in memory; restarting the container will lose this fallback configuration`,
    ...
    Object.keys(configFileJson).forEach((key) => {
        ...
        if (typeof value === 'string') {
            ...
            if (
                ...
                log.error(
                    `Invalid configuration for key: ${key}, expected a string but got ${typeof existingValue}`,
                );
                return getBaseConfig();
            }
            ...
            if (typeof existingValue === 'object') {
                ...
                log.error(
                    `Invalid configuration for key: ${key}, expected an object but got ${typeof existingValue}`,
                );
                return getBaseConfig();
            }
        } else {
            log.error(
                `Invalid configuration for key: ${key}, expected a string or an object but got ${typeof value}`,
            );
            return getBaseConfig();
        }
    });
...
export function getPrometheusConfiguration() {
    const prometheusConfig = getConfig().prometheus;
    const configurationSchema = joi.object().keys({
        enabled: joi.boolean().default(true),
    });
...
export function getPublicUrl(req: Request) {
    const publicUrl = getConfig().public?.url ?? process.env.WUD_PUBLIC_URL;
