...
test('should send message with correct text', async () => {
    ...
    slack.postMessage = jest.fn();
    await slack.trigger({} as Container);
    expect(slack.postMessage).toHaveBeenCalledWith('*Test Title*\n\nTest Body');
});
...
test('disabletitle should result in no title in message', async () => {
    ...
    expect(slack.postMessage).toHaveBeenCalledWith('Test Body');
});
